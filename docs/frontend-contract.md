...
本分支另显式校验公共 UI v1 与 `context.createPortal`；
公共 CSS/图标/兼容规范只在宿主
...
窄屏统一以 75vw 限宽，仅遇到更小的容器才进一步适配。
所有状态保留相同结构与尺寸，不因动作或正文长度收缩。
左侧 3rem 缩略图/文件图标等比例完整显示；中间单行文件名和单行状态，活动条使用固定预留位置；
右侧操作区为两个公共按钮触摸目标的宽度，至多两个动作；下载、移除和重试不挤在文字下方。
保持卡片总尺寸、三列和缩略图尺寸；为统一点击区域，文字列比旧 4rem 操作区布局更窄。
长名省略并尽量保留有界的扩展名，连续无空格、无扩展名和超长扩展名都不能撑破布局。
卡片主体是一个包含可见缩略图、标题和状态的原生按钮，空白区域也执行同一动作；
不再把无内容的透明按钮覆盖在它们上方。
有可预览媒体时直接打开媒体，并保留完整名称和必要的错误说明；其他文件才展示详情。
下载、移除、重试是独立按钮，不嵌套在预览按钮中，也不冒泡触发预览。
键盘焦点轮廓收在完整卡片内，关闭弹窗后不会在窄标题行绘制额外外框；不取消正常焦点返回。
草稿多项可换行，消息内排列继续由宿主决定，模块不覆盖宿主容器样式。
草稿提供移除和失败重试，聊天提供下载；图片点击卡片主体后才在可关闭的原生 dialog 中放大，
音视频通过显式播放入口展开，不默认撑大气泡，不内嵌 HTML/PDF。
卡片保持 phrasing `span`，可合法组合到 Markdown 段落/列表的行内插口。
...
迟到的旧弹窗 close 或媒体回调不能改写新资源。`aria-busy` 只描述卡片，不能代替
