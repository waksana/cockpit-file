...
test('compact styles are module-scoped, wrap multiple files and constrain long filenames', async () => {
  ...
  assert.match(css, /\.cf-card\s*\{[^}]*width:\s*17rem;[^}]*max-width:\s*100%/s);
  assert.match(css, /\.cf-thumbnail\s*\{[^}]*width:\s*3\.25rem;[^}]*height:\s*3\.25rem;/s);
  assert.match(css, /\.cf-card-name\s*\{[^}]*-webkit-line-clamp:\s*2;[^}]*overflow:\s*hidden;/s);
  assert.match(css, /\.cf-attachment-list\s*\{[^}]*flex-wrap:\s*wrap;/s);
  assert.doesNotMatch(css, /22rem|11rem|\b(?:body|html|:root)\b|\.chat-/);
